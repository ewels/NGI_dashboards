...
$(function () {
    ...
    try {
        ...
        sequencing_q_subtext = '<span style="color:#DF5353">'+pl['miseq_sequencing_queue_l']+'M</span>, '+
            ...
            '<span style="color:#DDDF0D">'+pl['novaseq_sequencing_queue_l']+'N</span> lanes in queue';
        ...
        sequencing_subtext = '<span style="color:#AF2323">'+pl['miseq_sequencing_l']+'M</span>, '+
            ...
            '<span style="color:#DDDF0D">'+pl['novaseq_sequencing_l']+'N</span> lanes in progress';
        ...
        make_balance_plot('#seq_balance',         pl_l['sequencing'],         [pl['miseq_sequencing_l'], pl['hiseq_sequencing_l'], pl['hiseqX_sequencing_l'], pl['novaseq_sequencing_l']],         undefined, sequencing_subtext);
...
function make_queue_plot(target, aim, now, subtext){
    try {
        ...
        if (!Array.isArray(now)){
            ...
            chroma_colors=chroma.scale(['#DF5353','#55BF3B','#7cb5ec', '#DDDF0D']).colors(now.length);
            series=[];
            for (i in now){
                obj={name:'serie'+i,
...
function make_balance_plot(target, aim, now, prev, subtext){
    try {
        ...
        if (!Array.isArray(now)){
            ...
            my_plotlines = Array();
            chroma_colors = chroma.scale(['#AF2323','#259F0B','#4c85cc','#DDDF0D']).colors(now.length);
            for (i in now){
                my_plotlines.push({
                    name:'serie'+i,
                    color : chroma_colors[i],
